...
Для замера скорости созданы несколько тестов. Создается коллекция на 100.000 элементов. Запускается 100 итераций. Берется среднее время.

Замеры скорости позволяют провести только сравнительную оценку. Они показывают, насколько те или иные методы быстрее других.

Так, встроенный метод итерации по коллекции **forEach** почти в 4 раза быстрее, чем через итератор **for...of**.

Добавление в коллекцию через метод **push** в 2-2.5 раза быстрее, чем через **add**.

Накладные расходы по сравнению с чистым Map составляют в среднем 10-15%.
